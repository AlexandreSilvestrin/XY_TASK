...
class Notas:
    ...
    def salvar_em_excel(self, df, nome_arquivo, localsalvar):
        ...
        self.printarInformacoes(f'DataFrame salvo em "{nome_arquivo}".', nomearq=nome_arquivo, status='success', mensagem='DataFrame salvo com sucesso')
    ...
    def salvarerro(self, dferro, empresa):
        ...
        with open(f"{self.local_salvar}/ERRO_{empresa}_{arq}.txt", "w") as arq:
            arq.write(f'{info}\n Arquivo: {arq}\n\n TRACEBACK:\n{erroF}')
        self.printarInformacoes(info, nomearq=f'ERRO_{empresa}_{arq}.txt', status='error', mensagem=info)

    def pegarCNPJS(self):
        #self.printarInformacoes('LIMPAR', nomearq='LIMPAR', status='info', mensagem='LIMPAR')
    ...
    def lerarquivos(self, pasta,Ctomados, Centrada, Cpdf):
        ...
        if Ctomados:
            ...
            if dftomados['df'] is None:
                ...
                self.printarInformacoes(f'''{empresa} SEM MOVIMENTO''', nomearq=f'{empresa}.xlsx', status='ok', mensagem='SEM MOVIMENTO')
    ...
    def gerarNotas(self):
        ...
        self.printarInformacoes(f'COMPLETOU NOTAS', nomearq='Notas', status='success', mensagem='Notas completadas com sucesso')

class Notasweb(Notas):
    def __init__(self, local, local_salvar, mes, ano, emit_log):
