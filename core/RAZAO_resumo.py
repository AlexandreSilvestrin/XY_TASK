...
class RAZAO:
    ...
    def resumir(self):
        ...
        if os.path.isdir(self.local):
            for arquivo in os.listdir(self.local):
        ...
        self.printarInformacoes(
            ...
            nomearq="",
            status="error",
            mensagem="O caminho fornecido não é válido.",
        )
        return False

    def printarInformacoes(self, conteudo, nomearq="", status="", mensagem=""):
        print(conteudo or mensagem)

    def gerar_resumo(self, local, locals, nomearq):
        try:
            return self._gerar_resumo(local, locals, nomearq)
        except Exception as exc:
            self.printarInformacoes(
                "",
                nomearq=nomearq,
                status="error",
                mensagem=f"ERRO AO GERAR ARQUIVO: {exc}",
            )
            return False

    def _gerar_resumo(self, local, locals, nomearq):
        ...
        df_iss = df[df['nome'].str.contains(" ISS ", case=False, na=False)].copy()
...
class RazaoWeb(RAZAO):
    def __init__(self, local, locals, emit_log):
