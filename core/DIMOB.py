...
def conversao_dimob(file_path, output_path):
    try:
        ...
        print(e)
        return False, f'Erro ao converter o arquivo: {e}', ''

def DimobWeb(entrada, saida, emit_log):
    try:
        success, message, output_file = conversao_dimob(entrada, saida)
        emit_log(
            module="importacao-dimob",
            status="success",
            file=output_file,
            message=f'Arquivo convertido e salvo em: {output_file}',
        )
        ...
        emit_log(
            ...
            status="error",
            file=output_file,
            message=f'Erro ao converter o arquivo: {e}',
        )
        return False, f'Erro ao converter o arquivo: {e}', ''

if __name__ == "__main__":
    a, e, nome = conversao_dimob(r"D:\Programacao\Python\MAURO\dimob\DIMOB NBB 2025- HENRIQUE.xlsx", r"C:\Users\Alexandre\Desktop\saida teste\dimob")